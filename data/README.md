[README.md](https://github.com/user-attachments/files/31987546/README.md)
...
## Five-year mortality prediction input

The five-year mortality prediction module requires no additional participant-level source file. It reuses the locked dynamic candidate generated by the mortality workflow:
...
- `interval_start_age`, `interval_end_age`, `interval_event`;
- `H_A`, `D_A`, `H_F`, `D_F`;
- `sex_baseline`, `education_z`, `calendar_year_c`.

The corresponding candidate lock, input gates and fixed scaling file must remain beside the RDS. The prediction module writes its age-eligible landmark RDS under `results/mortality_prediction/`, which is excluded from Git. Submission outputs contain only aggregate counts, estimates, confidence intervals and audit results.
